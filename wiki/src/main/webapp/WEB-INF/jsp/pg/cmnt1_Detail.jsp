...
/* $(function(){
	$("#reButton").click(function(){
		...
		location.href = "/reBoardReWrite.do?unq=${boardVO.unq}";
	});
}); */
function reButton(){
	location.href = "/recmnt1ReWrite.do?unq=${boardVO.unq}";
}
...
function fn_delete() {
	...
	if( confirm("정말 삭제?")  ) {
		location.href="/cmnt1Delete.do?unq=${boardVO.unq}";
	}
...
<%@ include file="/top/topmenu.jsp" %> 	
	...
	<form name="cmnt" method="post" action="/cmnt1DetailSave.do">
...
<table border="1" width="800">
	<tr>
		...
		<td><input type="text" name="title" size="90" value="${boardVO.title}"></td>
		</tr> 
		<%-- <tr>
		<th>암호</th>
		<td><input type="password" name="pwd" value="${boardVO.pwd}" disabled></td>
		</tr>  --%>
			<tr>
		<th style="background-color:#CEECF5;">글쓴이</th>
		<td><input type="text" name="name" size="90" value="${boardVO.name}" disabled></td>
		</tr>
			<tr>
		<th style="background-color:#CEECF5;">내용</th>
		<td>
		<textarea name="content" rows="5" cols="90">${boardVO.content}</textarea>
		</td>
		</tr>
		<tr>
		<th colspan="2">
			<button class='write_button' type="button" onclick="reButton()">답변</button>
			<button class='write_button' type="button" onclick="fn_action()">저장</button>
			<button class='write_button' type="reset"> 취소 </button>		
			<button class='write_button' type="button" onclick="fn_delete()">삭제</button>
